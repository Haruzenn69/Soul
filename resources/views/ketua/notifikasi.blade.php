...
@section('content')
    <div class="flex flex-col sm:flex-row justify-between items-start gap-3">
        <div>
            <h1 class="text-lg font-extrabold text-theme-dark">Notifikasi</h1>
            <p class="text-xs text-gray-400 mt-0.5">Pemberitahuan terkait ekskul yang anda pimpin</p>
        </div>
        @if($notifikasis->isNotEmpty())
            <form method="POST" action="{{ route('ketua.notifikasi.read-all') }}">
                @csrf
                <button type="submit" class="px-3.5 py-2 bg-gray-200 hover:bg-gray-300 text-gray-600 text-xs font-semibold rounded-lg transition">
                    Tandai Semua Dibaca
                </button>
            </form>
        @endif
    ...
    <div class="grid grid-cols-3 gap-4">
        ...
        <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm text-center">
            ...
            <p class="text-[11px] font-semibold text-gray-400">Belum Dibaca</p>
        </div>
        <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm text-center">
            <p class="text-2xl font-bold text-emerald-600">{{ $notifikasis->where('tipe', 'diterima')->count() }}</p>
            <p class="text-[11px] font-semibold text-gray-400">Masuk</p>
        </div>
    </div>
    ...
    <div class="space-y-3">
        @forelse($notifikasis as $notif)
            ...
            <div class="p-4 rounded-2xl border {{ $typeColor }} {{ $notif->is_read ? 'opacity-70' : '' }}">
                <div class="flex items-center gap-3">
                    <div class="flex items-center justify-center w-10 h-10 rounded-xl bg-white border border-gray-200 shrink-0">
                        {!! $typeIcon !!}
                    </div>
                    <div class="flex-1 min-w-0 flex items-center justify-between gap-2">
                        <h3 class="text-sm font-bold text-theme-dark leading-snug truncate">{{ $notif->judul }}</h3>
                        <span class="text-[10px] text-gray-400 shrink-0 whitespace-nowrap">{{ $notif->created_at->diffForHumans() }}</span>
                    </div>
                </div>
                <p class="text-xs text-gray-600 mt-2 leading-relaxed">{{ $notif->pesan }}</p>
                @if(!$notif->is_read)
        ...
        @empty
            <div class="p-12 text-center bg-white rounded-2xl border border-gray-100 shadow-sm">
                <div class="w-14 h-14 mx-auto bg-gray-100 rounded-2xl flex items-center justify-center text-gray-400 mb-3">
                ...
                <p class="text-xs text-gray-400 mt-1">Notifikasi pendaftaran, pengajuan keluar, dan laporan akan muncul di sini.</p>
            </div>
        @endforelse
    </div>
@endsection