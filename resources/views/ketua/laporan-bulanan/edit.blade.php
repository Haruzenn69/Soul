    ...
    <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm max-w-2xl">
        <form action="{{ route('ketua.laporan-bulanan.update', $laporan) }}" method="POST">
            ...
            <div class="mb-4 p-3 bg-blue-50 rounded-2xl border border-blue-100">
                <p class="text-[11px] text-blue-600 font-medium">📅 Laporan untuk periode <strong>{{ \Carbon\Carbon::createFromFormat('Y-m', $laporan->bulan)->translatedFormat('F Y') }}</strong>.</p>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="tujuan" rows="4" placeholder="Contoh: Mengembangkan kemampuan bermain alat musik..."
                    class="w-full px-4 py-2.5 rounded-2xl bg-gray-50 border border-gray-200 text-xs focus:outline-none focus:border-theme-blue transition">{{ old('tujuan', $laporan->tujuan) }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="evaluasi_keberhasilan" rows="3" placeholder="Contoh: Siswa telah mampu bermain bersama dengan koordinasi yang baik..."
                    class="w-full px-4 py-2.5 rounded-2xl bg-gray-50 border border-gray-200 text-xs focus:outline-none focus:border-theme-blue transition">{{ old('evaluasi_keberhasilan', $laporan->evaluasi_keberhasilan) }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="evaluasi_kendala" rows="3" placeholder="Contoh: Masih terdapat permasalahan dalam pengaturan sound..."
                    class="w-full px-4 py-2.5 rounded-2xl bg-gray-50 border border-gray-200 text-xs focus:outline-none focus:border-theme-blue transition">{{ old('evaluasi_kendala', $laporan->evaluasi_kendala) }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="evaluasi_solusi" rows="3" placeholder="Contoh: Akan dilaksanakan pelatihan dasar mengenai pengaturan sound..."
                    class="w-full px-4 py-2.5 rounded-2xl bg-gray-50 border border-gray-200 text-xs focus:outline-none focus:border-theme-blue transition">{{ old('evaluasi_solusi', $laporan->evaluasi_solusi) }}</textarea>
            </div>
            ...
            <div class="flex gap-2 mt-6">
                ...
                <a href="{{ route('ketua.laporan-bulanan.show', $laporan) }}" class="px-5 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 text-xs font-semibold rounded-full transition">Batal</a>
            </div>
        </form>
    </div>
@endsection