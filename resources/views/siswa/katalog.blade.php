    ...
    <div class="flex-1 flex flex-col min-w-0">
        ...
        <header class="px-4 md:px-8 py-4 bg-white/70 backdrop-blur-lg border-b border-sky-100 flex items-center justify-between gap-3 md:gap-4 sticky top-0 z-30">
            <div class="relative w-full max-w-md hidden sm:block">
                ...
                <input type="text" placeholder="Cari ekskul..." class="w-full pl-10 pr-4 py-2 bg-sky-50/70 border border-sky-100 rounded-xl text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white focus:border-sky-400 focus:ring-4 focus:ring-sky-100 transition-all">
            </div>
        ...
        <main class="p-4 md:p-8 space-y-6 overflow-y-auto">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-fade-up">
                <div>
                    <h1 class="text-xl font-extrabold text-slate-900">Katalog Ekskul</h1>
                    <p class="text-xs text-slate-400 mt-0.5">Temukan ekskul yang sesuai dengan minatmu</p>
                </div>
                @if(!$isRegistered && !$isPending)
                    <a href="{{ route('siswa.daftar-ekskul') }}" class="px-5 py-2.5 bg-gradient-to-r from-sky-400 to-blue-500 hover:from-sky-500 hover:to-blue-600 text-white font-semibold text-xs rounded-xl shadow-lg shadow-sky-200 transition-all flex items-center gap-2 hover:-translate-y-0.5">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @elseif($isPending)
                    <span class="px-4 py-2 bg-amber-100 text-amber-700 text-xs font-semibold rounded-lg border border-amber-200 flex items-center gap-1.5">
            ...
            <div class="flex gap-2 flex-wrap animate-fade-up" style="animation-delay: .1s">
                <button class="px-4 py-2 bg-gradient-to-r from-sky-400 to-blue-500 text-white text-xs font-semibold rounded-lg shadow-md shadow-sky-200">Semua</button>
                <button class="px-4 py-2 bg-white text-slate-600 text-xs font-semibold rounded-lg border border-sky-100 shadow-sm hover:bg-sky-50 transition">Olahraga</button>
                <button class="px-4 py-2 bg-white text-slate-600 text-xs font-semibold rounded-lg border border-sky-100 shadow-sm hover:bg-sky-50 transition">Seni</button>
                <button class="px-4 py-2 bg-white text-slate-600 text-xs font-semibold rounded-lg border border-sky-100 shadow-sm hover:bg-sky-50 transition">Akademik</button>
            </div>

            <!-- Daftar Ekskul -->
            <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 lg:gap-5">
                @forelse($ekskuls as $index => $ekskul)
                <div class="bg-white p-3 md:p-4 lg:p-5 rounded-2xl border border-sky-100 shadow-sm hover:shadow-lg hover:-translate-y-1 hover:border-sky-200 transition-all duration-300 animate-fade-up" style="animation-delay: {{ $index * 0.05 }}s">
                    <div class="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
                        <div class="w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 rounded-xl bg-gradient-to-br from-sky-400 to-blue-500 text-white font-bold flex items-center justify-center text-[10px] md:text-xs lg:text-sm shadow-lg shadow-sky-200 uppercase">
                            {{ substr($ekskul->nama_ekskul, 0, 2) }}
                        </div>
                        <div class="flex-1 min-w-0">
                            <h3 class="text-[11px] md:text-xs lg:text-sm font-bold text-slate-900 truncate">{{ $ekskul->nama_ekskul }}</h3>
                            <p class="text-[8px] md:text-[9px] lg:text-[10px] text-slate-400 truncate">Pembina: {{ $ekskul->pembina->nama ?? '-' }}</p>
                        </div>
                    </div>
                    <p class="text-[9px] md:text-[10px] lg:text-xs text-slate-600 mb-2 md:mb-3 line-clamp-2">{{ $ekskul->deskripsi ?? 'Deskripsi belum tersedia' }}</p>

                    @if($ekskul->is_open_recruitment)
                        <div class="mb-2 md:mb-3">
                            <span class="text-[8px] md:text-[9px] lg:text-[10px] font-semibold text-emerald-700 bg-emerald-50 border border-emerald-100 px-1.5 md:px-2 py-0.5 md:py-1 rounded-full">Buka Pendaftaran</span>
                            <p class="text-[8px] md:text-[9px] lg:text-[10px] text-slate-400 mt-1">{{ $ekskul->jadwal ?? 'Jadwal belum diatur' }}</p>
                        </div>
                        <a href="{{ route('siswa.form-daftar', $ekskul->id) }}" class="block w-full py-1.5 md:py-2 lg:py-2.5 bg-gradient-to-r from-sky-400 to-blue-500 hover:from-sky-500 hover:to-blue-600 text-white text-[9px] md:text-[10px] lg:text-xs font-semibold rounded-lg transition shadow-md shadow-sky-200 text-center hover:-translate-y-0.5">
                            Daftar Sekarang
                        </a>
                    @else
                        <div class="mb-2 md:mb-3">
                            <span class="text-[8px] md:text-[9px] lg:text-[10px] font-medium text-red-500 bg-red-50 px-1.5 md:px-2 py-0.5 md:py-1 rounded-full">Tidak Membuka Pendaftaran</span>
                            <p class="text-[8px] md:text-[9px] lg:text-[10px] text-slate-400 mt-1">{{ $ekskul->jadwal ?? 'Jadwal belum diatur' }}</p>
                        </div>
                        <button class="w-full py-1.5 md:py-2 lg:py-2.5 bg-slate-100 text-slate-400 text-[9px] md:text-[10px] lg:text-xs font-semibold rounded-lg cursor-not-allowed border border-slate-100" disabled>
                            Tidak Tersedia
                        </button>
                    @endif

                    <div class="mt-2 md:mt-3 pt-2 md:pt-3 border-t border-sky-50">
                        <a href="{{ route('ekskul.detail', $ekskul) }}" class="text-sky-600 text-[9px] md:text-[10px] lg:text-xs font-semibold hover:underline flex items-center gap-1">
                            Lihat Detail
                ...
                @empty
                <div class="col-span-2 lg:col-span-3 text-center py-12 text-slate-400">
