...
<body class="bg-gradient-to-br from-sky-50 via-white to-amber-50 text-slate-800 font-sans antialiased min-h-screen md:flex overflow-x-hidden">
    ...
    <aside class="w-64 bg-white/90 backdrop-blur border-r border-sky-100 shadow-sm flex-col justify-between p-5 hidden md:flex shrink-0 relative overflow-hidden">
    ...
    <div class="w-full flex-1 flex flex-col min-w-0">
        ...
        <header class="px-4 md:px-8 py-4 bg-white/70 backdrop-blur-lg border-b border-sky-100 flex items-center justify-between gap-3 md:gap-4 sticky top-0 z-30">
            <div class="flex items-center gap-3 flex-1 min-w-0">
                ...
                <form method="GET" action="{{ route('siswa.notifikasi') }}" class="relative w-full max-w-md hidden sm:block">
                    ...
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari..." class="w-full pl-10 pr-4 py-2 bg-sky-50/70 border border-sky-100 rounded-xl text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white focus:border-sky-400 focus:ring-4 focus:ring-sky-100 transition-all">
                </form>
            </div>
            ...
            <div class="flex items-center gap-2 md:gap-3 shrink-0">
                ...
                <a href="{{ route('siswa.notifikasi') }}" class="w-9 h-9 rounded-xl bg-gradient-to-r from-sky-100 to-blue-100 text-sky-700 border border-sky-200 flex items-center justify-center text-xs shadow-sm transition-colors">
        ...
        <main class="p-4 md:p-8 space-y-5 md:space-y-6 overflow-y-auto w-full max-w-3xl mx-auto">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 animate-fade-up">
                ...
                @if($notifikasis->isNotEmpty())
                    <form method="POST" action="{{ route('siswa.notifikasi.read-all') }}" class="w-full sm:w-auto">
                        @csrf
            ...
            <div class="grid grid-cols-3 gap-3 md:gap-4">
                <div class="bg-white p-3 md:p-4 rounded-2xl border border-sky-100 shadow-sm text-center animate-fade-up hover:-translate-y-1 hover:shadow-lg transition-all duration-300" style="animation-delay: .1s">
                    <p class="text-lg md:text-2xl font-extrabold text-sky-700">{{ $notifikasis->count() }}</p>
                    <p class="text-[8px] md:text-[11px] font-semibold text-slate-400">Total</p>
                </div>

                <div class="bg-white p-3 md:p-4 rounded-2xl border border-sky-100 shadow-sm text-center animate-fade-up hover:-translate-y-1 hover:shadow-lg transition-all duration-300" style="animation-delay: .2s">
                    <p class="text-lg md:text-2xl font-extrabold text-amber-600">{{ $notifikasis->where('is_read', false)->count() }}</p>
                    <p class="text-[8px] md:text-[11px] font-semibold text-slate-400">Belum Dibaca</p>
                </div>

                <div class="bg-white p-3 md:p-4 rounded-2xl border border-sky-100 shadow-sm text-center animate-fade-up hover:-translate-y-1 hover:shadow-lg transition-all duration-300" style="animation-delay: .3s">
                    <p class="text-lg md:text-2xl font-extrabold text-emerald-600">{{ $notifikasis->where('tipe', 'diterima')->count() }}</p>
                    <p class="text-[8px] md:text-[11px] font-semibold text-slate-400">Diterima</p>
                </div>
            </div>
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if(!$notif->is_read)
                                    <div class="mt-2.5 flex justify-end">
                                        <form method="POST" action="{{ route('siswa.notifikasi.read', $notif) }}" class="w-full sm:w-auto">
                                            @csrf
