...
<body class="bg-[#F8FAFC] text-slate-800 font-sans antialiased flex min-h-screen">
    ...
    <!-- SIDEBAR LEFT (drawer di mobile, statis di md+) -->
    <aside id="sidebar" class="w-64 bg-white border-r border-slate-200/80 flex-col justify-between p-4 md:p-5 shrink-0 fixed inset-y-0 left-0 z-50 transform -translate-x-full transition-transform md:translate-x-0 md:static md:flex">
        <div>
            <div class="flex items-center gap-3 mb-6 md:mb-8 px-2">
                <div class="w-9 h-9 md:w-10 md:h-10 rounded-2xl bg-blue-600 text-white flex items-center justify-center font-bold text-lg shadow-md shadow-blue-500/20">
                    SOUL
                </div>
                <div>
                ...
                <button id="closeSidebar" class="md:hidden ml-auto text-slate-400 hover:text-slate-600">
    ...
    <div class="flex-1 flex flex-col min-w-0">
        ...
        <header class="px-4 md:px-8 py-3 md:py-4 bg-white border-b border-slate-200/80 flex items-center justify-between gap-3">
            <!-- Mobile: hamburger -->
            <button id="menuBtn" class="md:hidden w-9 h-9 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-center text-slate-600">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex items-center gap-2 md:gap-3">
                ...
                <a href="{{ route('siswa.notifikasi') }}" class="w-9 h-9 rounded-xl bg-blue-50 border border-blue-200 flex items-center justify-center text-xs relative text-blue-600 transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="bg-red-50 hover:bg-red-100 text-red-600 px-2.5 py-2 md:px-4 md:py-2 rounded-lg text-xs font-semibold border border-red-100 transition shadow-sm">
                        Logout
                    </button>
                </form>
            </div>
        ...
        <main class="p-4 md:p-8 space-y-5 md:space-y-6 overflow-y-auto w-full max-w-3xl mx-auto">

            <!-- Header -->
            <div class="flex flex-col sm:flex-row justify-between items-start gap-3">
                <div>
                    <h1 class="text-lg md:text-xl font-bold text-slate-900">Notifikasi</h1>
                    <p class="text-xs text-slate-400 mt-0.5">Pemberitahuan status pendaftaran ekskulmu</p>
                </div>
                @if($notifikasis->isNotEmpty())
                    <form method="POST" action="{{ route('siswa.notifikasi.read-all') }}">
                        @csrf
                        <button type="submit" class="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-semibold rounded-lg transition w-full sm:w-auto">
                            Tandai Semua Dibaca
                        </button>
                    </form>
                @endif
            ...
            <div class="grid grid-cols-3 gap-3 md:gap-4">
                ...
                <div class="bg-white p-3.5 md:p-4 rounded-xl border border-slate-200/70 shadow-sm text-center">
                    <p class="text-lg md:text-2xl font-bold text-blue-600">{{ $notifikasis->where('is_read', false)->count() }}</p>
                    <p class="text-[10px] md:text-[11px] font-semibold text-slate-400">Belum Dibaca</p>
                </div>
                <div class="bg-white p-3.5 md:p-4 rounded-xl border border-slate-200/70 shadow-sm text-center">
                    <p class="text-lg md:text-2xl font-bold text-emerald-600">{{ $notifikasis->where('tipe', 'diterima')->count() }}</p>
                    <p class="text-[10px] md:text-[11px] font-semibold text-slate-400">Diterima</p>
                </div>
            </div>
            ...
            <div class="space-y-3 md:space-y-4">
                @forelse($notifikasis as $notif)
                    @php
                        ...
                        $typeIcon = match($notif->tipe) {
                            ...
                            default => '<svg class="w-4 h-4 md:w-5 md:h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>',
                        };
                    @endphp
                    <div class="p-3.5 md:p-4 rounded-xl md:rounded-2xl border {{ $typeColor }} {{ $notif->is_read ? 'opacity-70' : '' }}">
                        <div class="flex items-center gap-3">
                            <div class="flex items-center justify-center w-9 h-9 md:w-10 md:h-10 rounded-xl bg-white border border-slate-200 shrink-0">
                                {!! $typeIcon !!}
                            </div>
                            <div class="flex-1 min-w-0 flex items-center justify-between gap-2">
                                <h3 class="text-xs md:text-sm font-bold text-slate-900 leading-snug truncate">{{ $notif->judul }}</h3>
                        ...
                        @if(!$notif->is_read)
                            <div class="mt-2.5 flex justify-end">
                                <form method="POST" action="{{ route('siswa.notifikasi.read', $notif) }}">
                                    @csrf
                                    <button type="submit" class="text-[10px] font-semibold text-blue-600 hover:text-blue-700 px-2.5 py-1.5 bg-white border border-blue-200 rounded-lg w-full sm:w-auto text-center">
                                        Tandai dibaca
                                    </button>
                                </form>
                            </div>
                ...
                @empty
                    <div class="p-8 md:p-12 text-center bg-white rounded-2xl border border-slate-200/70 shadow-sm">
                        <div class="w-12 h-12 md:w-14 md:h-14 mx-auto bg-slate-100 rounded-2xl flex items-center justify-center text-slate-400 mb-3">
                            <svg class="w-6 h-6 md:w-7 md:h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>
                            </svg>
                        </div>
                        <p class="text-sm font-medium text-slate-600">Belum ada notifikasi</p>
                        <p class="text-xs text-slate-400 mt-1">Notifikasi status pendaftaran ekskul akan muncul di sini.</p>
                    </div>
                @endforelse
            </div>
